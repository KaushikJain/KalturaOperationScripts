...
if ($partnerId && $syncDuration && $kmcPartnerId && $kmcSecretKey)
{
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://rest-sgs1.ott.kaltura.com/api_v3/service/OTTUser/action/anonymousLogin',
...
",
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://rest-sgs1.ott.kaltura.com/api_v3/service/asset/action/list',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
            ...
            if (sizeof($emptyImagelist) > 0)
            {
                ...
                foreach ($emptyImagelist as $key => $value)
                {
                    ...
                    curl_setopt_array($curl, array(
                        ...
                        CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                        CURLOPT_CUSTOMREQUEST => 'POST',
                        CURLOPT_POSTFIELDS => "{\"text\":\"THIS IS FOR STG. DONT PANIC.Images were missing for $key - $value. Triggered the redistribution. \"}",
                        CURLOPT_HTTPHEADER => array(
                            'Content-type: application/json'
                        ) ,
                    ));
